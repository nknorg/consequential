...
type RunJob func(ctx context.Context, workerID, jobID uint32) (result interface{}, success bool)

// FinishJob finish a job with job id, returns whether success
type FinishJob func(ctx context.Context, jobID uint32, result interface{}) (success bool)
...
func (cs *ConSequential) Start(ctx context.Context) error {
	cs.initJobChan()
	cs.ctx = ctx
...
func (cs *ConSequential) tryJob(workerID, jobID uint32) bool {
	result, success := cs.RunJob(cs.ctx, workerID, jobID)
	if !success {
...
func (cs *ConSequential) tryFinishJobs() uint32 {
	...
	for {
		...
		success = cs.FinishJob(cs.ctx, jobID, result)
		if success {
			cs.shiftRingBuf()
			numFinished++
		} else {
